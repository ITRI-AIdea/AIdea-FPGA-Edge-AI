...
def write_tfrec(tfrec_filename, image_dir, img_list):
    ...
    with tf.io.TFRecordWriter(tfrec_filename) as writer:
        ...
        for img in img_list:
            ...
            image_raw = tf.image.decode_png(image_png, channels=3)
            image = tf.image.encode_jpeg(image_raw, format='rgb', quality=100)

            # get the shape of the image from the jpg file header
